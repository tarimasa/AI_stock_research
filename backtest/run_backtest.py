...
# ── 設定 ──────────────────────────────────────────────────────────────────────
BACKTEST_START = os.environ.get("BACKTEST_START", "2025-04-01")
BACKTEST_END   = os.environ.get("BACKTEST_END",   "2026-03-31")
...
# ── 取引コスト（往復） ────────────────────────────────────────────────────────
# 日本株の短期売買における現実的コスト:
#   - 手数料: ネット証券で約定代金の0.05〜0.1%（片道）、定額プランなら実質ゼロの場合も
#   - スプレッド: 流動性が高い東証プライム銘柄で片道 0.05〜0.1%
#   - 合計往復コストは約 0.2〜0.4%
# 環境変数で上書き可能:
#   COMMISSION_PCT_ONEWAY=0.05   (%、片道)
#   SLIPPAGE_PCT_ONEWAY=0.05     (%、片道)
COMMISSION_PCT_ONEWAY = float(os.environ.get("COMMISSION_PCT_ONEWAY", 0.05))
SLIPPAGE_PCT_ONEWAY   = float(os.environ.get("SLIPPAGE_PCT_ONEWAY",   0.05))
ROUND_TRIP_COST_PCT = (COMMISSION_PCT_ONEWAY + SLIPPAGE_PCT_ONEWAY) * 2

# ── 保有モード ────────────────────────────────────────────────────────────────
# "overnight"  : 翌日始値でエントリー、当日引けで決済（現行ロジック、日足前提）
# "half_day"   : 翌日始値でエントリー、(始値+終値)/2 で決済（後場寄付手仕舞い近似）
#                TP/SL は引き続きザラ場中に判定。半日保有 EV を概算したい場合に利用。
HOLDING_MODE = os.environ.get("HOLDING_MODE", "overnight").lower()
...
def generate_report(df: pd.DataFrame, analysis: dict) -> None:
    """
    ...
    print("\n" + "=" * 70)
    print("バックテスト結果レポート")
    print(f"  対象期間: {BACKTEST_START} 〜 {BACKTEST_END}")
    ...
    print(f"\n【全体統計（Stage1通過銘柄）】")
    print(f"  総シグナル数: {overall.get('n', 0):,}件")
    print(f"  TP達成率(+{TP_PCT}%到達): {overall.get('tp_rate', 0):.1f}%")
    print(f"  SL発動率({SL_PCT}%到達):  {overall.get('sl_rate', 0):.1f}%")
    print(f"  勝率(return>0):         {overall.get('win_rate', 0):.1f}%")
    print(f"  期待値(平均リターン):   {overall.get('expected_value', 0):+.3f}%")
    print(f"  中央値リターン:         {overall.get('median_return', 0):+.3f}%")
    ...
    print(f"\n【シグナル別閾値分析】  ★ = n≥30 かつ期待値>0")
    for key, label in signal_labels.items():
        if key in analysis and isinstance(analysis[key], list):
            _print_bin_table(label, analysis[key])

    # ブレイクアウト
    if "breakout_5d" in analysis:
        bo = analysis["breakout_5d"]
        print("\n  ブレイクアウト有無:")
        for flag, stats in bo.items():
            print(
                f"    {flag:>5}: n={stats['n']:>5} "
                f"TP={stats['tp_rate']:.1f}% SL={stats['sl_rate']:.1f}% "
                f"期待値={stats['expected_value']:+.3f}%"
            )
    ...
    save_cols = [c for c in save_cols if c in df_pass.columns]
    df_pass[save_cols].to_csv(results_path, index=False, encoding="utf-8-sig")
    print(f"\n詳細データ: {results_path}  ({len(df_pass):,}行)")

    # 閾値サマリー
    rows = []
    for signal, bins_data in analysis.items():
...
def run_backtest(
    ...
    # バックテスト期間に絞り込み
    df_test = df_results[
        (df_results["Date"] >= start_dt) &
        (df_results["Date"] <= end_dt) &
        df_results["next_open"].notna()
    ].copy()

    print(
        f"[backtest] テスト期間: {len(df_test):,}行 | "
...
def main():
    parser = argparse.ArgumentParser(description="株式スクリーナー バックテスト")
    ...
    if args.analyze:
        ...
        if df_results.empty:
            print("[backtest] データがありません。")
            return
        analysis = analyze_thresholds(df_results)
        generate_report(df_results, analysis)
