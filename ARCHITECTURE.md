...
## 8. セットアップ進捗状況（2026/03/29時点）

> **次の担当者向け引き継ぎメモ**
> Windows WSL 環境で作業を継続する場合はこのセクションを参照してください。
> 作業ブランチ: `claude/implement-architecture-guide-P2YAR`

### 完了済み ✅

| ステップ | 内容 | 備考 |
|---------|------|------|
| コード実装 | 全モジュール実装・プッシュ済み | セキュリティ修正3件含む |
| Azure リソースグループ | `rg-stock-bot` 作成済み | `japaneast` |
| Azure Storage Account | `stockbotstoragetarimasa` 作成済み | 既存アカウントを流用 |
| Azure Storage Container | `stock-bot` 作成済み | portfolio.json 保存先 |
| Azure Container Registry | `stockbotacrtarimasa` 作成済み | Admin ユーザー有効化済み |
| Docker イメージビルド | `stockbotacrtarimasa.azurecr.io/stock-bot-webhook:latest` | ACR にプッシュ済み |
| Container Apps 環境 | `stock-bot-env` 作成済み | `Microsoft.OperationalInsights` 登録済み |
| Container Apps デプロイ | `stock-bot-webhook` デプロイ済み | FQDN 取得済み |
| LINE Webhook URL 設定 | Container Apps の FQDN を LINE Developers Console に設定済み | |

### 残作業 ⬜

| ステップ | 内容 | 手順 |
|---------|------|------|
| ⬜ LIFF フォームデプロイ | `src/liff/` を Azure Static Web Apps に配置 | 下記「LIFF デプロイ手順」参照 |
| ⬜ LIFF 登録 | LINE Developers Console で LIFF ID 発行 | 下記「LIFF 登録手順」参照 |
| ⬜ GitHub Secrets 登録 | Actions に各種シークレット設定 | 下記「GitHub Secrets」参照 |
| ⬜ 動作確認 | GitHub Actions を手動実行して LINE 通知を確認 | `workflow_dispatch` で実行 |

-----

## 9. GitHub Secrets / Azure / LINE LIFF 設定手順
...
|`AZURE_STORAGE_CONNECTION_STRING`|Azure Portal > Storage Account `stockbotstoragetarimasa` > アクセスキー |
...
### Azure Container Apps デプロイ手順（実績コマンド）

> 以下は実際に使用したコマンド。リソース名は tarimasa 環境用に修正済み。

```bash
# 1. Azure Container Registry 作成
az acr create --name stockbotacrtarimasa --resource-group rg-stock-bot --sku Basic
az acr update --name stockbotacrtarimasa --admin-enabled true

# 2. Docker イメージをビルドして ACR に push
#    ※ リポジトリルート（Dockerfile がある場所）で実行すること
cd ~/claude-workspace/AI_stock_research
az acr build --registry stockbotacrtarimasa --image stock-bot-webhook:latest .

# 3. Container Apps 環境作成
#    ※ Microsoft.OperationalInsights が未登録の場合は先に登録
az provider register -n Microsoft.OperationalInsights --wait
az provider register -n Microsoft.App --wait
az containerapp env create --name stock-bot-env --resource-group rg-stock-bot --location japaneast

# 4. ACR 認証情報を取得
az acr credential show --name stockbotacrtarimasa --query "passwords[0].value" -o tsv

# 5. Container App デプロイ
az containerapp create \
  --name stock-bot-webhook \
  --resource-group rg-stock-bot \
  --environment stock-bot-env \
  --image stockbotacrtarimasa.azurecr.io/stock-bot-webhook:latest \
  --ingress external --target-port 8000 \
  --registry-server stockbotacrtarimasa.azurecr.io \
  --registry-username stockbotacrtarimasa \
  --registry-password "<4で取得したパスワード>" \
  --env-vars LINE_CHANNEL_ACCESS_TOKEN=... LINE_CHANNEL_SECRET=... \
             AZURE_STORAGE_CONNECTION_STRING=...

# 6. 取得した FQDN を LINE Developers Console の Webhook URL に設定
#    例: https://stock-bot-webhook.xxx.japaneast.azurecontainerapps.io/webhook
```

### LIFF フォームデプロイ手順（Azure Static Web Apps）
...
# 2. デプロイトークン取得
az staticwebapp secrets list --name stock-bot-liff --query "properties.apiKey" -o tsv

# 3. src/liff/ の内容をデプロイ
#    （GitHub Actions または SWA CLI を使用）
npm install -g @azure/static-web-apps-cli
swa deploy ./src/liff --deployment-token <2で取得したトークン> --env production

# 4. デプロイされた URL を確認
az staticwebapp show --name stock-bot-liff --query "defaultHostname" -o tsv
#    → https://<hostname>.azurestaticapps.net
#    この URL を LIFF のエンドポイント URL に設定する
```

### コード変更点（実装後の修正サマリ）

初期実装から以下の修正を加えた。次の担当者は修正済みの状態から作業開始できる。

| コミット | 内容 |
|---------|------|
| `c0a9eb4` | 全モジュール初期実装 |
| `10be653` | セキュリティ修正: `/portfolio` 認証・CORS・入力バリデーション・起動時チェック |
| `5d323bb` | `webhook_server.py` の環境変数チェックを lifespan に移動（pytest 対応）|
